...
def process_and_store(items: list) -> int:
    ...
    for item in items:
        try:
            # دوباره چک garbage بعد از ترجمه هم
            if is_garbage(item["title_original"], item.get("description_original", "")):
                continue
            title_fa = translate_to_persian(item["title_original"])
            if is_garbage(title_fa, ""):
                continue
            desc_fa = translate_to_persian(item["description_original"]) if item.get("description_original") else ""
...
def run_fetch_cycle():
    ...
    unsent = sorted(get_unsent_news(20), key=lambda x: x["published_at"])
    for news in unsent:
        if is_garbage(news.get("title_fa", "") or news.get("title_original", ""), ""):
            mark_sent(news["id"])  # رد کردن زباله‌های قبلی
            continue

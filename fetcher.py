...
def is_garbage(title: str, description: str = "") -> bool:
    """فقط محتوای خطای واقعی را رد می‌کند (نه توضیح کوتاه)"""
    ...
    if any(b in t for b in bad):
        return True
    # فقط اگر عنوان وجود داشته باشد و خیلی کوتاه/بی‌معنی باشد
    clean = (title or "").strip()
    if clean and len(clean) < 12 and clean.lower().startswith("error"):
        return True
    return False
...
def process_and_store(items: list) -> int:
    ...
    for item in items:
        try:
            ...
            # اگر توضیح Error بود خالی بگذار، وگرنه نگه دار
            if desc_fa and is_garbage(desc_fa, desc_fa):
                desc_fa = ""

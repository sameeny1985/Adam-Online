...
def is_garbage(title: str, description: str = "") -> bool:
    """فیلتر پیام‌های خطا و بی‌معنی (مثل Error 500 گوگل)"""
    ...
    try:
        ...
        for entry in feed.entries[:35]:
            ...
            if is_garbage(title, summary):
                continue
            if force_filter or source.get("type") in ("international", "x_account", "diaspora"):
...
def send_to_telegram(item: dict) -> bool:
    """ارسال با requests مستقیم (بدون async مشکل‌دار)"""
    ...
    try:
        ...
        if r.status_code == 200 and r.json().get("ok"):
            logger.info(f"Telegram OK: {item['title_fa'][:50]}")
            return True
        else:

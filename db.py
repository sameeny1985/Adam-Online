...
def news_exists(unique_hash: str) -> bool:
    conn = get_connection()
    c = conn.cursor()
    c.execute("SELECT 1 FROM news WHERE unique_hash = ?", (unique_hash,))
    exists = c.fetchone() is not None
    conn.close()
    return exists

def insert_news(item: dict) -> bool:
    """درج خبر جدید. اگر تکراری باشد False برمی‌گرداند."""
    conn = get_connection()
    c = conn.cursor()
    try:
        c.execute("""
            INSERT INTO news (
                unique_hash, title_original, title_fa,
                description_original, description_fa,
                link, source_name, published_at, fetched_at, sent_to_telegram
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
        """, (
            item["unique_hash"],
            item["title_original"],
...
def get_unsent_news(limit: int = 20):
    conn = get_connection()
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("""

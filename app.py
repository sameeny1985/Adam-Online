...
(function() {
    ...
    for (var i = 0; i < sentences.length; i++) {
      ...
      if ((buf + " " + s).trim().length <= maxLen) {
        ...
        if (s.length <= maxLen) {
          ...
          for (var j = 0; j < s.length; j += maxLen) {
            parts.push(s.slice(j, j + maxLen));
          }
          buf = "";
        }
      }
  ...
  // TTS فارسی با Google Translate (کار روی گوشی و کامپیوتر)
  function speakPersian(text) {
    return new Promise(function(resolve) {
      ...
      function playNext() {
        ...
        var q = encodeURIComponent(chunks[idx]);
        var url = "https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=fa&q=" + q;
        ...
        audio.onerror = function() {
          // اگر گوگل بلاک کرد، برو قطعه بعد
          idx += 1;
          playNext();
        };
  ...
  async function readNext() {
    ...
    if (current >= cards.length) {
      stopReader();
      setStatus("پایان خواندن");
      return;
    }
    var card = cards[current];
    var title = card.getAttribute("data-title") || "";
    var desc = card.getAttribute("data-desc") || "";
    var text = cleanText(title + (desc ? ". " + desc : ""));
    setStatus("در حال خواندن خبر " + (current + 1) + " از " + cards.length + "…");
    highlight(current);
    await speakPersian(text);
    if (!reading) return;
    current += 1;
  ...
  async function startReader() {
    ...
    if (!cards.length) {
      setStatus("خبری برای خواندن نیست");
      return;
    }
    reading = true;
    current = 0;
    ...
    setStatus("شروع خبرخوان…");
    await speakPersian("شروع خبرخوان");
    if (!reading) return;
    readNext();
  }

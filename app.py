...
def trigger():
    """
    فچ اجباری.
    برای امنیت می‌توانید هدر X-Secret بفرستید.
    """
    secret = os.getenv("FETCH_SECRET", "")
    if secret and request.headers.get("X-Secret") != secret:
        # اگر secret تنظیم شده باشد و مطابقت نداشته باشد، رد می‌کند
        # اگر secret خالی باشد، اجازه می‌دهد (برای تست آسان‌تر)
        if secret:
    ...
    try:
        ...
        return jsonify({
            "status": "ok",
            "added": count,
            "message": "Fetch finished. Check Telegram channel and site."
        })

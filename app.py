...
function getPersianVoice() {
  ...
  // ترجیح صدای فارسی
  let v = voices.find(x => x.lang.startsWith('fa'));
  if (!v) v = voices.find(x => x.lang.startsWith('ar')); // fallback عربی نزدیک
...
function readNext() {
  if (!reading || current >= cards.length) {
    stopReader();
    return;
  }
  ...
  document.getElementById('readerStatus').textContent = 'در حال خواندن خبر ' + (current + 1) + ' از ' + cards.length;
  highlight(current);
  speakText(text, () => {
    current++;
    if (reading) readNext();
...
function startReader() {
  ...
  if (!cards.length) {
    document.getElementById('readerStatus').textContent = 'خبری برای خواندن نیست';
    return;
  }
  reading = true;
  current = 0;
  ...
  // بعضی مرورگرها voices را دیر لود می‌کنند
  if (synth.getVoices().length === 0) {
    synth.onvoiceschanged = () => readNext();
  } else {
    readNext();
  }
}
...
function stopReader() {
  ...
  document.getElementById('readerBtn').classList.remove('playing');
  document.getElementById('readerLabel').textContent = 'خبرخوان';
  document.getElementById('readerIcon').textContent = '🔊';
  document.getElementById('readerStatus').textContent = '';
}

function toggleReader() {
  if (reading) stopReader();
  else startReader();
}

...
def cleanup_garbage():
    ...
    try:
        ...
        c.execute("""
            ...
            lower(title_original) LIKE '%error 500%' OR
            lower(title_fa) LIKE '%error 500%' OR
            lower(title_original) LIKE '%server error%' OR
            lower(title_fa) LIKE '%server error%' OR
            lower(title_original) LIKE '%that''s an error%' OR
            lower(title_fa) LIKE '%that''s an error%' OR
            lower(description_original) LIKE '%error 500%' OR
            lower(description_fa) LIKE '%error 500%'
        """)
        deleted = c.rowcount
        conn.commit()
        conn.close()
